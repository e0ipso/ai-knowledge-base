  ...
  'node_modules/gray-matter/node_modules/js-yaml/lib/js-yaml/dumper.js'(exports2, module2) {
    ...
    function isPlainSafe2(c, prev) {
      return (
        ...
        c !== 65279 &&
        c !== CHAR_COMMA2 &&
        c !== CHAR_LEFT_SQUARE_BRACKET2 &&
        c !== CHAR_RIGHT_SQUARE_BRACKET2 &&
        c !== CHAR_LEFT_CURLY_BRACKET2 &&
        c !== CHAR_RIGHT_CURLY_BRACKET2 &&
        c !== CHAR_COLON2 &&
        (c !== CHAR_SHARP2 || (prev && isNsChar(prev)))
      );
    }
    function isPlainSafeFirst2(c) {
      return (
        ...
        c !== CHAR_ASTERISK2 &&
        c !== CHAR_EXCLAMATION2 &&
        c !== CHAR_VERTICAL_LINE2 &&
        c !== CHAR_EQUALS2 &&
        c !== CHAR_GREATER_THAN2 &&
        c !== CHAR_SINGLE_QUOTE2 &&
        c !== CHAR_DOUBLE_QUOTE2 &&
        c !== CHAR_PERCENT2 &&
        c !== CHAR_COMMERCIAL_AT2 &&
        c !== CHAR_GRAVE_ACCENT2
      );
    }
...
// src/harnesses/opencode/hooks/kk-lint-tick.ts
init_cjs_shims();
...
// src/lib/agents-block.ts
var AGENTS_BLOCK_START = '<!-- >>> kenkeep:kk-index >>> -->';
var AGENTS_BLOCK_END = '<!-- <<< kenkeep:kk-index <<< -->';
var AGENTS_POINTER = [
...
function checkAgentsKkBlock(root, kkDir, nodeCount) {
  const issues = [];
  const agentsFile = (0, import_node_path6.join)(root, 'AGENTS.md');
  if (!(0, import_node_fs6.existsSync)(agentsFile)) {
    issues.push({
      file: 'AGENTS.md',
      message:
  ...
  const body = (0, import_node_fs6.readFileSync)(agentsFile, 'utf8');
  ...
  if (
    nodeCount > 0 &&
    !(0, import_node_fs6.existsSync)((0, import_node_path6.join)(kkDir, 'ENTRY.md'))
  ) {
    issues.push({
...
function readRedirectsLedger(nodesDir) {
  const file = (0, import_node_path7.join)(nodesDir, REDIRECTS_FILENAME);
  if (!(0, import_node_fs7.existsSync)(file)) return {};
  try {
    const parsed = RedirectsLedgerSchema.safeParse(
      JSON.parse((0, import_node_fs7.readFileSync)(file, 'utf8'))
    );
...
function runLint(opts) {
  ...
  for (const dir of foldersUnder(opts.nodesDir)) {
    if (!(0, import_node_fs8.existsSync)((0, import_node_path8.join)(dir, INDEX_FILENAME))) {
      errors.push({
        rule: 'missing-folder-index',
        file: dir,
        message: `folder ${import_node_path8.posix.normalize((0, import_node_path8.relative)(opts.nodesDir, dir).split(import_node_path8.sep).join(import_node_path8.posix.sep)) || '.'} has no index.md`,
...
function foldersUnder(nodesDir) {
  if (!(0, import_node_fs8.existsSync)(nodesDir)) return [];
  const out = [];
  const walk = dir => {
    out.push(dir);
    for (const entry of (0, import_node_fs8.readdirSync)(dir, { withFileTypes: true })) {
      if (entry.isDirectory()) walk((0, import_node_path8.join)(dir, entry.name));
    }
  };
  walk(nodesDir);
  return out;
...
function lintStateFile(stateDir) {
  return (0, import_node_path10.join)(stateDir, 'lint-state.json');
}
...
async function runLintTick(startCwd, harnessTag) {
  ...
  if (!(0, import_node_fs10.existsSync)(paths.installedVersionFile)) return;
  try {
    ...
    process.stderr.write('\u{1F50D} kenkeep Lint: Running knowledge base lint\u2026\n');
    ...
    process.stderr.write('\u{1F9F9} kenkeep Lint: Knowledge base lint complete.\n');
  } catch (err) {
