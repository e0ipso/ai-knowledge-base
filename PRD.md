...
2. When the session ends - or when context compaction is about to fire - a hook captures a slice of the transcript into `.ai/kenkeep/_sessions/`, marked pending for proposal extraction. (Capture writes the slice as-is to the gitignored staging area; there is no automated redaction - see Goal 6.)
3. Proposal extraction turns each pending session log into structured `practice`/`map` candidates. For the Codex, Cursor, OpenCode, and Copilot adapters this runs in the background on the next session start (the `kk-proposal-drain` hook spawns the harness's headless binary, without blocking the session). For the Claude adapter the drain hook is intentionally a no-op: proposals are extracted inline at the start of a `kk-curate` run instead. Either way the run's stream-json trace lands in `_logs/proposal/`.
...
2. A `SessionStart` hook injects the current `ENTRY.md`. Delivery differs per harness: Claude and Cursor inject it as session context directly; OpenCode writes it to `.opencode/AGENTS.md`; Copilot writes it into a managed sentinel block in `.github/copilot-instructions.md`.
