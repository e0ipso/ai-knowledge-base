...
/**
 * Default filename of the per-repo KB scope file at the repository root.
 */
export const KBIGNORE_FILENAME = '.kbignore';

/**
 * Renders the default `.kbignore` stub from a list of harness adapters.
 *
...
export function renderKbignoreStub(adapters: HarnessAdapter[]): string {
  ...
  const lines: string[] = [];
  // 1. Header.
  lines.push('# .kbignore — per-repo scope for the AI knowledge-base doc scan.');
  lines.push('#');
  ...
  lines.push('#           this file. Editing `.kbignore` cannot opt them back in;');
  ...
  // 3. Harness instruction directories — uncommented.
  ...
  lines.push('# include that surface in the KB scan.');
  for (const d of dirs) {
    lines.push(d);
  }
  lines.push('');
...
/**
 * Writes `<repoRoot>/.kbignore` from the current harness registry if and
 ...
 *
 * Never overwrites — a user-edited `.kbignore` is sacred.
 */
export function ensureKbignore(repoRoot: string): { written: boolean; path: string } {
  const path = join(repoRoot, KBIGNORE_FILENAME);
  if (existsSync(path)) {
