---
name: kb-curate
description: Curate pending session logs into knowledge-base nodes by running the `npx @e0ipso/ai-knowledge-base curate --harness codex` CLI, then resolve any contradictions surfaced by the curator with the user in-session. Use when the user wants to process accumulated session captures, or when the SessionStart nudge reports pending session logs.
---
...
Run `npx @e0ipso/ai-knowledge-base curate --harness codex` in the project root. The command:
...
- If `npx @e0ipso/ai-knowledge-base curate --harness codex` reports `locked`, do not retry; explain that another curate run is in progress.
