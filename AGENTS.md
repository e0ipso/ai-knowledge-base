...
- **Bootstrap scope is controlled exclusively by `.kbignore` at the repo root.** `bootstrap-incremental` always walks the repo root; there are no CLI flags for include/exclude/walk-root selection. `init` writes a `.kbignore` stub on first run (and on `init --upgrade` when the file is missing) that denies the registered harnesses' instruction directories. Harness memory ingestion (`CLAUDE.md` and friends, via `listMemoryFiles()`) bypasses `.kbignore` entirely. See the README's migration table for rewriting old flag-based invocations.
- **[Do not run `curate` or `bootstrap-incremental` in CI.](.ai/knowledge-base/nodes/practice/practice-dont-run-llm-pipelines-in-ci.md)** They are LLM-driven and human-supervised. When you must invoke `bootstrap-incremental` non-interactively, pass `--yes` to skip the confirmation gate.
