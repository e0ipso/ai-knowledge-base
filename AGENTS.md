...
[Every node's `id` equals `<kind>-<slug>`, and its filename is `<id>.md`.](.ai/kenkeep/nodes/practice/practice-lint-naming-rules.md) The leaf lives in a topical folder under `nodes/`, not a `kind` bucket; lint asserts filename/id agreement, that every leaf carries a stable id, and that every folder under `nodes/` has an `index.md` (the old kind/directory agreement check is gone). `relates_to` / `depends_on` must resolve to an existing node id; dangling edges are errors. See [map-node-frontmatter](.ai/kenkeep/nodes/map/map-node-frontmatter.md) and [map-nodes-directory](.ai/kenkeep/nodes/map/map-nodes-directory.md).
