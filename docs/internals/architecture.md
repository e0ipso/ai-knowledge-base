...
Leaf nodes (the documents) live in topical folders under `.ai/kenkeep/nodes/` at any depth. Every folder carries a generated `index.md` (an **index node**): a deterministic table-of-contents rollup of its child leaves and immediate subfolders, ordered by graph in-degree then title. The top-level catalog `ENTRY.md` (the SessionStart entry point) is a purpose-built whole-tree launchpad — totals plus the branch list — distinct from the per-folder `nodes/index.md`; `GRAPH.md` is the full edge listing.
