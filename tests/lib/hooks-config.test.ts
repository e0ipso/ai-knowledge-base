...
describe('writeClaudeHookConfig', () => {
    ...
    expect(parsed.hooks.Stop).toEqual([
      {
        hooks: [
          {
            type: 'command',
            command: 'node "$CLAUDE_PROJECT_DIR/.claude/hooks/kb-capture.mjs"',
          },
        ],
      },
    ]);
  ...
  it('preserves foreign hooks (commands without the .claude/hooks/kb- prefix)', async () => {
    ...
    expect(commands).toContain('node "$CLAUDE_PROJECT_DIR/.claude/hooks/kb-capture.mjs"');
  });
  ...
  it('emits async: true only when the spec sets it', async () => {
    await writeClaudeHookConfig(root, [
      ...
      { event: 'SessionStart', scriptPath: '.claude/hooks/kb-proposal-drain.mjs', async: true },
    ]);
    ...
    const sessionStart = entries
      .flatMap(e => e.hooks)
      .find(h => h.command.includes('kb-session-start.mjs'));
    const drain = entries
      .flatMap(e => e.hooks)
      .find(h => h.command.includes('kb-proposal-drain.mjs'));
    ...
    await writeClaudeHookConfig(root, [
      {
        ...
        scriptPath: '.claude/hooks/kb-filter.mjs',
        matcher: '**/*.md',
      },
    ]);
    const parsed = JSON.parse(readFileSync(settingsFile, 'utf8'));
  ...
  it('throws when the existing settings.json is unparseable', async () => {
    ...
    await expect(
      writeClaudeHookConfig(root, [{ event: 'Stop', scriptPath: '.claude/hooks/kb-capture.mjs' }])
