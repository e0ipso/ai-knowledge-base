...
describe('codex kk-capture hook (spawned)', () => {
  let sandbox: string;
  let codexHome: string;

  beforeEach(async () => {
    sandbox = makeSandbox();
    codexHome = makeSandbox('ai-kk-codex-home-');
  ...
  it('compiled hook bundle exists at dist/hooks/codex/kk-capture.cjs', () => {
  ...
  it('exits silently with KENKEEP_BUILDER_INTERNAL=1 (recursion guard)', async () => {
    ...
    const result = await runHook(
      ...
      { KENKEEP_BUILDER_INTERNAL: '1', CODEX_HOME: codexHome }
    );
  ...
  it('writes a session log from the dated rollout', async () => {
    ...
    const log = readFileSync(
      join(sandbox, '.ai/kenkeep/_sessions', logs[0] as string),
      'utf8'
    );
