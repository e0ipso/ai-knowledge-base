...
const hookPath = join(repoRoot, 'dist/hooks/cursor/kk-capture.cjs');
...
describe('cursor kk-capture hook (spawned)', () => {
  let sandbox: string;
  ...
  it('compiled hook bundle exists at dist/hooks/cursor/kk-capture.cjs', () => {
  ...
  it('writes a session log from transcript_path', async () => {
    ...
    const body = readFileSync(join(sandbox, '.ai/kenkeep/_sessions', sessions[0]!), 'utf8');
...
function readdirSessions(root: string): string[] {
  const dir = join(root, '.ai/kenkeep/_sessions');
