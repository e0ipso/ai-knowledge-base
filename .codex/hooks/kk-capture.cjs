  ...
  main: async payload => {
    ...
    try {
      ...
      const homeRoot =
        ...
        (0, import_node_path7.join)((0, import_node_os.homedir)(), '.codex');
...
function locateRollout(homeRoot, sessionId) {
  const sessionsRoot = (0, import_node_path7.join)(homeRoot, 'sessions');
...
function sessionsDirForDate(sessionsRoot, when) {
  ...
  return (0, import_node_path7.join)(sessionsRoot, y, m, d);
}
function findByFilename(dir, sessionId) {
  ...
  for (const name of entries) {
    if (name.startsWith('rollout-') && name.endsWith(suffix)) {
      return (0, import_node_path7.join)(dir, name);
    }
  }
  return null;
}
