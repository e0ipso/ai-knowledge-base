...
**Why:** the knowledge base is per-repo and short-lived in any single user's working copy; the population is small and re-init is cheap. Migrators add code paths that have to be tested forever and tend to silently paper over genuinely-breaking semantic changes.
