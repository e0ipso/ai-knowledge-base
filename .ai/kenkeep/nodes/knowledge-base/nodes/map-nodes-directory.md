...
confidence: high
summary: >-
  Knowledge nodes live as markdown under nodes/{practice,map}/<id>.md. Two
  kinds: practice (how we build) and map (what exists).
---

# `nodes/` directory and the two kinds

Every kept fact is a markdown file under `.ai/kenkeep/nodes/` with YAML frontmatter. The directory has exactly two subdirectories, one per kind:
