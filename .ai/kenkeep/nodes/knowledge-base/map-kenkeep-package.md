...
summary: >-
  ...
  redacted slices, lets a curator write nodes/, injects INDEX into every new
  session.
---
...
A `SessionStart` hook injects `INDEX.md` into every new AI session so the harness starts each conversation with the team's accumulated context. The knowledge base itself is plain markdown — readable, diffable, reviewable like code.
