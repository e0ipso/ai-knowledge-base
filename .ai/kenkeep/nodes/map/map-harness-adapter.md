...
Active-harness resolution priority (in `src/harnesses/detect.ts`): explicit `--harness <id>` flag, then env detection (`CLAUDECODE=1` for Claude; `CURSOR_VERSION` for Cursor; Codex and OpenCode export no marker), then `cliDefaultHarness` in `config.yaml`, then the first registered harness. The same priority is mirrored inside skills via the materialized `/tmp/kk-detect-harness.mjs` helper (with an early `CLAUDECODE=1` return so Claude wins over Cursor when both env signals leak).
