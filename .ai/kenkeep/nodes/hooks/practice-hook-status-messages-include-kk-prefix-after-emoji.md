...
This convention applies across all four harness adapters (Claude, Codex, Cursor, OpenCode). The prefix helps users distinguish messages originating from the knowledge base system from those of other tools or extensions. Error diagnostics that use `PACKAGE_TAG` (`[kenkeep]`) are not affected by this rule.
