...
The Cursor adapter in this project currently relies on `additional_context` to inject the KB index and queue status into the model's context. As a result, the model in Cursor does not actually receive the KB index via the hook — it only receives it if the user has the static pointer in AGENTS.md and opens the INDEX.md file explicitly.
