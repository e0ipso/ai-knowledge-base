...
depends_on: []
confidence: high
summary: "Committed project settings at .ai/knowledge-base/config.yaml. Strict: unknown keys are a hard error."
---

# `config.yaml`

Project-level settings at `.ai/knowledge-base/config.yaml`. Committed. Strict: any unknown key causes a hard error naming the offending file.
...
- `bootstrapModel` — passed on `bootstrap-incremental` spawns. Also honored on a best-effort basis by the agent-driven `/kb-bootstrap` skill, but the skill ignores `bootstrapModel.effort` because the `Task` tool has no `effort` parameter.
