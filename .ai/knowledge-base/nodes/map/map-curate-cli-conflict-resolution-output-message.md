...
summary: >-
  ...
  the three resolution outcomes and pointing users at /kb-curate.
---
`src/commands/curate.ts` (around lines 102-110) emits a multi-line message whenever the curate run produces conflict files. The message starts with a `log.warn` headline ("N conflict(s) need resolution in .ai/knowledge-base/conflicts/.") and follows with per-outcome lines for accept, reject, and keep-as-record, plus a note that unresolved conflicts re-surface on the next curate run.
