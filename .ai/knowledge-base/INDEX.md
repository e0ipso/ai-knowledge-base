...
_42 nodes • 42 currently valid • 0 superseded • last updated 2026-05-12T15:50:15.081Z_


## Practice (how we build)
- **Rename Stage 1/Stage 2 to Transcript/Proposal across the KB pipeline** — Rename Stage 1 to Transcript and Stage 2 to Proposal everywhere (code, config, frontmatter, prompts, docs). [`nodes/practice/practice-rename-stage-vocab-to-transcript-proposal.md`] (tags: naming, refactor, kb-pipeline, vocabulary)
...
- **Session logs and extraction logs are intermediate artifacts, safe to wipe** — `_sessions/` and `_logs/stage-2/` are gitignored intermediate artifacts; curated nodes under `nodes/` are the durable output. [`nodes/practice/practice-sessions-and-stage2-logs-are-intermediate-artifacts.md`] (tags: cleanup, kb-pipeline, intermediate-artifacts)
...
- **Proposal: structured candidate nodes extracted from a Transcript** — The 'Proposal' is the structured set of practice/map candidate nodes produced by extraction from a Transcript (formerly 'Stage 2'). [`nodes/map/map-proposal-artifact.md`] (tags: kb-pipeline, artifact, vocabulary)
- **Transcript: raw session capture in the KB pipeline** — The 'Transcript' is the raw role-tagged session log fed into the KB extraction pipeline (formerly 'Stage 1'). [`nodes/map/map-transcript-artifact.md`] (tags: kb-pipeline, artifact, vocabulary)
...
- **kb-stage2-drain: async worker that runs the extraction step** — SessionStart async hook that spawns a Claude SDK subprocess to extract structured proposals from each queued session log. [`nodes/map/map-kb-stage2-drain.md`] (tags: worker, stage-2, kb-pipeline)
...
_15 additional nodes hidden by token budget — see GRAPH.md_
