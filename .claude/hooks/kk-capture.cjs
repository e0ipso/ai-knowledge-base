...
// src/harnesses/claude/hooks/kk-capture.ts
var kk_capture_exports = {};
__export(kk_capture_exports, {
  CLAUDE_EVENT_TO_TRIGGER: () => CLAUDE_EVENT_TO_TRIGGER,
});
...
// src/harnesses/claude/transcript.ts
init_cjs_shims();
...
function extractContentBlockReads(text, toolNames, pathKey) {
  const out = [];
  for (const rawLine of text.split('\n')) {
    const line = rawLine.trim();
...
var CLAUDE_READ_TOOLS = /* @__PURE__ */ new Set(['Read']);
function extractClaudeReads(text) {
  return extractContentBlockReads(text, CLAUDE_READ_TOOLS, 'file_path');
}

// src/harnesses/claude/hooks/kk-capture.ts
var PACKAGE_TAG = '[kenkeep]';
var CLAUDE_EVENT_TO_TRIGGER = {
  Stop: 'stop',
...
runHookEntry({
  ...
  main: async payload => {
    ...
    try {
      ...
      const eventName =
        typeof payload['hook_event_name'] === 'string' ? payload['hook_event_name'] : void 0;
      const trigger =
        (eventName !== void 0 ? CLAUDE_EVENT_TO_TRIGGER[eventName] : void 0) ?? 'stop';
      const input = {
        session_id: sessionId,
      ...
      process.stdout.write(
        `${JSON.stringify({ systemMessage: '\u{1F4BE} kenkeep Capture: Session transcript saved.' })}
`
      );
    } catch (err) {
      process.stderr.write(
