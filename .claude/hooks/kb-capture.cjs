  ...
  "node_modules/rc-config-loader/lib/rc-config-loader.js"(exports2) {
    ...
    function findConfig({ parts, loaderByExt, loadersByOrder, configFileName, packageJSON, packageJSONFieldName }) {
      ...
      while (extensions.length) {
        ...
        const configLocation = join7(parts, configFileName + ext);
      ...
      if (packageJSON) {
        const pkgJSONLoc = join7(parts, "package.json");
...
function extractText(content) {
  ...
  if (Array.isArray(content)) {
    return content.filter((c2) => !!c2 && typeof c2 === "object").filter((c2) => (c2.type ?? "text") === "text").map((c2) => typeof c2.text === "string" ? c2.text : "").filter((s) => s.length > 0).join("\n");
  }
  return "";
}
