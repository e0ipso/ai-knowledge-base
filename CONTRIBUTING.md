...
A "harness" is one of the assistant CLIs we drive (Claude Code, Codex CLI, Cursor, OpenCode, ...). Each harness lives under `src/harnesses/<id>/` and ships as a `HarnessAdapter` implementation. Codex and Cursor are the shell-hook reference adapters (`.codex/hooks.json` and `.cursor/hooks.json`); OpenCode uses a plugin shim. To wire up a new one (call it `<id>`):
